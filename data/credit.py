...
def get_credits(course_code):
    html = get_html(
        'https://www.polymtl.ca/etudes/cours/recherche/%2A?sigle=' + course_code)
    ...
    trim = trim_html(sub_html, '<span class="tooltip triplet">',
...
with open("courses_credit.txt", "w") as courses_credit_file:
    for code in sorted(courses):
        course = courses[code]
        courses_credit_file.write(
            code + " " + str(course['credit']) + " " + course['title'] + "\n")
